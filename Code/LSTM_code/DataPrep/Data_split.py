...
def dicts_to_list():
    ...
    with open("./books_list.json", "w+") as fp:
        json.dump(blist, fp)
    ...
    blist.clear()
    rev = open("./review_data.json", "r")
    line = rev.readline()
    while line:
        blist.append(json.loads(line))
        line = rev.readline()

    with open("./reviews_list.json", "w+") as fp:
        json.dump(blist, fp)
...
def train_valid_split(n=10):
    filename = "./train_data.json"

    print("Loading review data...")
    fp = open(filename, "r")
    tdata = json.load(fp)
    print("data size %d" % len(tdata))
    print("selecting test ids..." + str(n) + "% of total data")
    # test_ids = random.sample(userids, int(len(userids) * n / 100))
    data_train, data_valid = tts(tdata, test_size=n/100, shuffle=True)

    valid_file = open("./valid_data.json", "w+")
    train_file = open("./train8_data.json", "w+")

    print("Creating test and training file...")
    json.dump(data_train, train_file)
    json.dump(data_valid, valid_file)
